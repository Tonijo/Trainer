...
function fillSticker(x, y, colour) {
    ...
    ctx.fillRect(stickerSize * x,stickerSize * y, stickerSize-1, stickerSize-1);
    
}
function fillStickerUpFace(x,y,colour) {
	var offset=50-(y)*25;
	var ajustementvertical= stickerSize-stickerSize*coefficient3D;
	var offsetvertical= (2-(y-1))*ajustementvertical;
	if(document.getElementById("cubeType").value == "2x2"){
		offsetvertical=offsetvertical+20;
		offset=offset+25;
	}
	ctx.fillStyle = colour;
	ctx.beginPath();
	ctx.moveTo(offset+stickerSize*x+25,y*stickerSize+offsetvertical);
	ctx.lineTo(offset+stickerSize*x+25+stickerSize-2,y*stickerSize+offsetvertical);
	ctx.lineTo(offset+stickerSize*x+stickerSize-2,y*stickerSize+offsetvertical+stickerSize*coefficient3D-1);
	ctx.lineTo(offset+stickerSize*x,y*stickerSize+offsetvertical+stickerSize*coefficient3D-1);
	ctx.lineTo(offset+stickerSize*x+25,y*stickerSize+offsetvertical);
	ctx.fill();
}

function fillStickerRightFace(x,y,colour) {
	var offset;
	var ajustementvertical= stickerSize-stickerSize*coefficient3D;
	var offsetvertical;
	if (y <= 2) {
		offset=50-y*25;
		offsetvertical= (2-(y-1))*ajustementvertical;
	}
	
	else{
		offset=0;
		offsetvertical= -stickerSize*coefficient3D+1;
	}
	...
	if (x==6) {
		offset=-50;
		offsetvertical= -8-stickerSize*coefficient3D;
	}
	
	if((document.getElementById("cubeType").value == "2x2")&&(x>=4)){
		offsetvertical=offsetvertical+20;
		offset=offset-25;
	}
	...
	ctx.fillStyle = colour;
        ctx.beginPath();
	ctx.moveTo(offset+stickerSize*x+25,y*stickerSize+offsetvertical);
	ctx.lineTo(offset+stickerSize*x+25,y*stickerSize+offsetvertical+stickerSize-2);
	ctx.lineTo(offset+stickerSize*x+1,y*stickerSize+offsetvertical+stickerSize+stickerSize*coefficient3D-1-2);
	ctx.lineTo(offset+stickerSize*x+1,y*stickerSize+offsetvertical+stickerSize*coefficient3D-1);
	ctx.lineTo(offset+stickerSize*x+25,y*stickerSize+offsetvertical);
	ctx.fill();
}
...
function fillWithIndex(x, y, face, index, cubeArray, shouldBeCleared = false) {
    ...
    if(face=="u" ) {
	fillStickerUpFace(x, y, colour);
    }
    else if (face=="r") {
	fillStickerRightFace(x, y, colour);
    }
    else if (face=="f") {
...
if(document.getElementById("cubeType").value == "2x2"){
  ...
  fillWithIndex(4, 3, "r", 3, cubeArray);
  ...
  //moi
  fillWithIndex(4, 4, "r", 9, cubeArray);
...
//rest of right face
  fillWithIndex(5, 4, "r", 8, cubeArray);
  fillWithIndex(5, 3, "r", 5, cubeArray);
  fillWithIndex(6, 4, "r", 9, cubeArray);
  fillWithIndex(6, 3, "r", 6, cubeArray);
...
listener.simple_combo("z", function() {	doAlg("B");});
...
listener.simple_combo("n", function() {	doAlg("r'");});
listener.simple_combo("c", function() {	doAlg("l");});
listener.simple_combo("r", function() {	doAlg("l'");});
listener.simple_combo("p", function() {	doAlg("u");});
listener.simple_combo("a", function() {	doAlg("u'");});
listener.simple_combo("`", function() {	doAlg("M");});
listener.simple_combo("y", function() {	doAlg("M");});
listener.simple_combo("b", function() {	doAlg("M'");});
listener.simple_combo("t", function() {	doAlg("x");});
listener.simple_combo("v", function() {	doAlg("x'");});
listener.simple_combo("m", function() {	doAlg("y");});
listener.simple_combo("q", function() {	doAlg("y'");});
listener.simple_combo("x", function() {	doAlg("z");});
listener.simple_combo("w", function() {	doAlg("z'");});
